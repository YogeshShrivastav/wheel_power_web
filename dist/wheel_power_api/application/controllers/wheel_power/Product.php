...
class Product extends APP_Controller {
	...
	public function products_list()
	{
        ...
        $brand_str = '';
        $array = array();
        for($i=0;$i<count($_POST['data']['brand']);$i++)
        {
            $array[]="whp_products.brand = '".$_POST['data']['brand'][$i]."' ";
        }
        ...
        $this->db->select('whp_products.id,whp_products.brand,whp_products.category,whp_products.sub_category,whp_products.product_name,whp_products.cat_no,whp_products.price,whp_product_feature.feature,whp_product_feature.value');
        $this->db->from('whp_products');
        ...
        if(isset($_POST['data']["category"]) && $_POST['data']["category"]!='all' )
        {
            $this->db->where("whp_products.category ='".$_POST['data']["category"]."'");
        }
        if(isset($_POST['data']['search']) && $_POST['data']['search'])
        {
            $this->db->group_start();
            $this->db->like("whp_products.product_name",$_POST['data']['search']);
            $this->db->or_like("whp_products.category",$_POST['data']['search']);
            $this->db->or_like("whp_products.cat_no",$_POST['data']['search']);
            $this->db->or_like("whp_products.brand",$_POST['data']['search']);
            $this->db->group_end();
        }
		// $this->db->limit('20',$_POST['start']);
        ...
        foreach ($result as $key)
		{
            ...
            $image = $this->db->select("image")->from("whp_product_image")->where('product_id',$key['id'])->where('del',"0")->get()->row_array();
