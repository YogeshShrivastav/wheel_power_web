...
class order extends MY_Controller {
    ...
    public function order_list()
    {
        ...
        {
            $this->db->select("whp_order.date_created,whp_order.status,whp_order.id as order_id,whp_customer.*,whp_user.name as created_by_name,whp_warehouse.warehouse_name");
        ...
        {
            $this->db->select("whp_order_packing.date_created,whp_order_packing.order_id as order_id,whp_order_packing.id as packing_id,whp_order_packing.item,whp_customer.*");
            $this->db->from("whp_order_packing");
    ...
    public function varify_payment()
    {
        ...
        $this->db->query("update whp_order_payment_mode set varify = 'verified' where id='".$id."' limit 1");
        
        $order_id = $this->db->select("order_id")->from("whp_order_payment_mode")->where("id",$id)->where("del","0")->get()->row_array();
    ...
    public function get_assign_order()
    {
        ...
        $order_list=array();
        if($data['page_type'] == 'packing_pend' ||$data['page_type'] == 'dispatch_pend') 
        {
            $this->db->select("whp_order.*,whp_order.id as order_id,whp_customer.*,COUNT(whp_order_packing.order_id) as packing_count,SUM(whp_order_packing.item) as packed_item");
            $this->db->from("whp_order");
            $this->db->join("whp_customer","whp_order.customer_id = whp_customer.id","left");
            $this->db->join("whp_order_packing","whp_order_packing.order_id = whp_order.id","left");
            $this->db->where("whp_order.del","0");
            $this->db->group_by("whp_order.id");
            $this->db->where("whp_order.status","assign");
            ...
            {
                $this->db->where("whp_order.packing_flag","1");
            }
        ...
        {
            $this->db->select("whp_order_packing.*,whp_customer.*,COUNT(whp_order_packing.order_id) as packing_count,SUM(whp_order_packing.item) as packed_item");
            $this->db->from("whp_order_packing");
            $this->db->join("whp_customer","whp_order_packing.customer_id = whp_customer.id","left");
            $this->db->where("whp_order_packing.del","0");
            $this->db->group_by("whp_order_packing.order_id");
